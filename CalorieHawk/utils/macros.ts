...
function pickBase(): string {
  ...
  const envBase = (process?.env?.EXPO_PUBLIC_MACRO_API as string) || process?.env?.MACRO_API_BASE;
  if (envBase) return envBase.replace(/\/+$/, "");

  // Fallback to app.json/app.config.ts -> extra.MACRO_API_BASE if set
  const extra = (Constants.expoConfig?.extra as any) ?? {};
  if (extra.MACRO_API_BASE) return String(extra.MACRO_API_BASE).replace(/\/+$/, "");
  ...
  return "http://<YOUR-LAN-IP>:8000"; // ← change to your machine’s LAN IP if testing on device
}
...
function makeKey(q: string, grams: number | undefined, includeSurvey: boolean): CacheKey {
  return `${q.toLowerCase().trim()}|g=${grams ?? -1}|s=${includeSurvey ? 1 : 0}`;
}
...
function roundOrNull(v: number | null | undefined, digits = 2): number | null {
  if (v === null || v === undefined || Number.isNaN(v)) return null;
  const m = 10 ** digits;
  return Math.round((v as number) * m) / m;
}
...
export async function getMacros(
  ...
  const res = await fetch(url.toString(), {
    ...
    if ((e as any)?.name === "AbortError") throw new Error("Macro service request timed out");
  ...
  if (!res.ok) {
    ...
    const msg = detail ? `Macro service error ${res.status}: ${detail}` : `Macro service error ${res.status}`;
    throw new Error(msg);
  }

  const json = (await res.json()) as MacroServiceResponse;

  // normalize/round to keep UI tidy
  json.per_100g = {
...
export async function fetchMacros(query: string, grams: number, includeSurvey = false) {
